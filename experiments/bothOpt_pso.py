# Ablation Study: Optimizing alphas and omegas together
...
n_particles = args.parts
outer_iter = "ablation2" # should be one string without '_'
...
def objective_bothOpt(inner_iter, particle_idx, alpha_vars, omega_vars):
...
print(f"=== ABLATION2: Starting {opt_type} with alpha, omega both Opt({n_particles} particles and {n_iterations} iterations) ===")

start_opt_time = time.time()
# cost: list, pos: dict, cost_history: dict, z01_loss_history: dict, swarm: list
best_cost, best_pos, cost_history, z01_loss_history, penalty_history, best_iteration_record, swarm = bothOpt_pso_per_case_with_nan_control(
    objective_fn=objective_bothOpt, n_particles=n_particles, n_iterations=n_iterations,
    case1_alpha_bounds=case1_alpha_pso_bounds, case2_alpha_bounds=case2_alpha_pso_bounds, case3_alpha_bounds=case3_alpha_pso_bounds,
    case1_omega_bounds=case1_param_pso_bounds, case2_omega_bounds=case2_param_pso_bounds, case3_omega_bounds=case3_param_pso_bounds,
    case1_alpha_names=case1_alpha_names, case2_alpha_names=case2_alpha_names, case3_alpha_names=case3_alpha_names,
    case1_omega_names=case1_param_names, case2_omega_names=case2_param_names, case3_omega_names=case3_param_names,
    neg_z_fom=neg_z_fom, nan_z_fom=nan_z_fom,
    custom_alpha_initializer=custom_alpha_init
)

end_opt_time = time.time()
opt_elapsed = end_opt_time - start_opt_time

case1_z01_loss_history = z01_loss_history['case1']
case2_z01_loss_history = z01_loss_history['case2']
case3_z01_loss_history = z01_loss_history['case3']

case1_penalty_history = penalty_history['case1']
case2_penalty_history = penalty_history['case2']
case3_penalty_history = penalty_history['case3']

case1_all_names = case1_alpha_names + case1_param_names
case2_all_names = case2_alpha_names + case2_param_names
case3_all_names = case3_alpha_names + case3_param_names

case1_best_all = dict(zip(case1_all_names, best_pos['case1']))
case2_best_all = dict(zip(case2_all_names, best_pos['case2']))
case3_best_all = dict(zip(case3_all_names, best_pos['case3']))

case1_final_cost, case2_final_cost, case3_final_cost = best_cost

case1_best_inner, case1_best_particle = best_iteration_record['case1']
case2_best_inner, case2_best_particle = best_iteration_record['case2']
case3_best_inner, case3_best_particle = best_iteration_record['case3']
...
case_bests = {
    "case1": case1_best_all,
    "case2": case2_best_all,
    "case3": case3_best_all
}

case_alpha_names = {
    "case1": case1_alpha_names,
    "case2": case2_alpha_names,
    "case3": case3_alpha_names
}

case_omega_names = {
    "case1": case1_param_names,
    "case2": case2_param_names,
    "case3": case3_param_names
}

final_selected_case = min(case_costs, key=case_costs.get)
final_expected_cost = case_costs[final_selected_case]
final_best = case_bests[final_selected_case]

final_alphas = {k:v for k,v in final_best.items() if k in case_alpha_names[final_selected_case]}
...
final_omegas_w_vb_frac = {k:v for k,v in final_best.items() if k in case_omega_names[final_selected_case]}
final_omegas = convert_Vb_from_Vb_frac(final_omegas_w_vb_frac)
...
# 1) voltage_data
case1_best_voltage_csv = abla2_read_best_voltage_csv(case="case1", best_iteration=f"{outer_iter}_{case1_best_inner}_{case1_best_particle}", tb_date=tb_date, folder=".")
case1_voltage_filename = os.path.basename(case1_best_voltage_csv)
case2_best_voltage_csv = abla2_read_best_voltage_csv(case="case2", best_iteration=f"{outer_iter}_{case2_best_inner}_{case2_best_particle}", tb_date=tb_date, folder=".")
case2_voltage_filename = os.path.basename(case2_best_voltage_csv)
case3_best_voltage_csv = abla2_read_best_voltage_csv(case="case3", best_iteration=f"{outer_iter}_{case3_best_inner}_{case3_best_particle}", tb_date=tb_date, folder=".")
...
for i, filename in enumerate([case1_voltage_filename, case2_voltage_filename, case3_voltage_filename]):
    match = re.search(rf'_voltage_data_([^_]+_\d+_\d+)_({tb_date})\.csv', filename)
    if match:
        if i == 0:
            case1_outer_inner_part = match.group(1)
            _, inner_iter, particle = case1_outer_inner_part.split('_')
            case1_final_best_sim = f"{outer_iter}_{inner_iter}_{particle}"
        elif i == 1:
            case2_outer_inner_part = match.group(1)
            _, inner_iter, particle = case2_outer_inner_part.split('_')
            case2_final_best_sim = f"{outer_iter}_{inner_iter}_{particle}"
        elif i == 2:
            case3_outer_inner_part = match.group(1)
            _, inner_iter, particle = case3_outer_inner_part.split('_')
            case3_final_best_sim = f"{outer_iter}_{inner_iter}_{particle}"
    if not match:
...
# 2) sim_output
case1_best_sim_output_csv = read_best_sim_output_csv(best_iteration=f"{case1_final_best_sim}", tb_date= tb_date, folder=".")
case2_best_sim_output_csv = read_best_sim_output_csv(best_iteration=f"{case2_final_best_sim}", tb_date= tb_date, folder=".")
case3_best_sim_output_csv = read_best_sim_output_csv(best_iteration=f"{case3_final_best_sim}", tb_date= tb_date, folder=".")
...
if final_selected_case == "case1":
    ...
    final_best_iter = case1_final_best_sim
    ...
    soft_selection_sim_output_dict = soft_selection_sim_output_df.iloc[0].to_dict()
    final_best_iter = case2_final_best_sim
    ...
    soft_selection_sim_output_dict = soft_selection_sim_output_df.iloc[0].to_dict()
    final_best_iter = case3_final_best_sim
...
with open(soft_selection_csv, 'w') as f:
    ...
    f.write(f"{final_selected_case},{final_best_iter}, {final_expected_cost}\n")
    f.write("Number of Particles, Number of Iterations\n")
    f.write(f"{n_particles}, {n_iterations}\n")
    f.write("Condition, Softmax Temperature\n")
...
plt.figure(figsize=(12, 6))
plt.plot(cost_history_df['case1'], marker='o', markersize=5, label='Case 1', color='blue')
plt.plot(x_z01, case1_z01_loss_history, marker='x', markersize=5, label='Case 1 Z01 Loss', color='#66B2FF', linestyle='--')
plt.plot(cost_history_df['case2'], marker='o', markersize=5, label='Case 2', color='orange')
plt.plot(x_z01, case2_z01_loss_history, marker='x', markersize=5, label='Case 2 Z01 Loss', color='#FFB84D', linestyle='--')
plt.plot(cost_history_df['case3'], marker='o', markersize=5, label='Case 3', color='green')
...
if task in ["dualBW", "dualGain"]:
    # n_outer = len(cost_history_df['case1'])
    ...
    plt.plot(x_z01, case1_penalty_history, marker='s', markersize=5, label='Case 1 Penalty', color='#004C99', linestyle=':')
    plt.plot(x_z01, case2_penalty_history, marker='s', markersize=5, label='Case 2 Penalty', color='#994C00', linestyle=':')
    plt.plot(x_z01, case3_penalty_history, marker='s', markersize=5, label='Case 3 Penalty', color='#006633', linestyle=':')
...
plt.title("Best Cost per Case vs PSO Iteration")
plt.xlabel("PSO Iteration (n_iteration)")
...
for case in cases:
    ...
    with open(out_path, "w") as f:  
        writer = csv.writer(f)
        writer.writerow(["iter", "z01_loss"])  
        for iter, z01_value in enumerate(z01_histories[case], start=1):
            if z01_value is None:
                writer.writerow([f"{iter}", "NaN"])
            else:
                writer.writerow([f"{iter}", float(z01_value)])
...
with open(hard_selection_csv, "w") as f:
    ...
    f.write(f"{case1_final_best_sim}, {case2_final_best_sim}, {case3_final_best_sim}, {hard_selected_cost:.2f}, {final_expected_cost:.2f}, {abs(hard_selected_cost - final_expected_cost):.2f}\n")
    f.write("Number of Iterations, Number of Particles, Elapsed time for optimization (s)\n")
    f.write(f"{n_iterations}, {n_particles}, {opt_elapsed:.2f}\n")
