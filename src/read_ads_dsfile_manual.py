...
def read_dsfile_manual(final_stage_dsfile, selected_case, iteration, task, tb_date):

    if task == "dualBW": #target freq: fc1, fc2 & wideband BW
        output_dict = {
        f"{selected_case}_cg_bw_fc1" : [],  # Hz
        f"{selected_case}_cg_bw_fc2" : [],  # Hz
        f"{selected_case}_cg_fc1" : float("nan"), # linear
        f"{selected_case}_cg_fc2" : float("nan"), # linear
        }

    elif task == "dualGains": # maximize gain across all frequencies
        output_dict = {
            f"{selected_case}_cg_fc1": [], # list of linear value
            f"{selected_case}_cg_fc2": [], # list of linear value
        }

    elif task == "dualGainsBW":
        output_dict = {
            f"{selected_case}_cg_bw_fc1": [], # len 2 list
            f"{selected_case}_cg_bw_fc2": [], 
            f"{selected_case}_cg_fc1": [], # list of linear value
            f"{selected_case}_cg_fc2": [],
        }
        ...
        output_dict = {
            f"{selected_case}_cg_fc1": float("nan"),
        }
    ...
    try:
        ...
        for member in list(output_data.keys()):
            ...
            if (f"{selected_case}_Vin_fund" in block_cols) and ("RFfreq" in block_cols): # ALL VIN_FUND data are in the same block, DC results don't have RFfreq
                freq_vin_fund = (block_data["RFfreq"]*1e9).round(2) # Hz
                vin_fund = block_data[f"{selected_case}_Vin_fund"] # V

                vin_fund_data = pd.DataFrame({
                    "freq": freq_vin_fund,
                    f"{selected_case}_Vin_fund": vin_fund
                })

            elif (f"{selected_case}_Vout_fund" in block_cols) and ("RFfreq" in block_cols):
                freq_vout_fund = (block_data["RFfreq"]*1e9).round(2) # Hz
                vout_fund = block_data[f"{selected_case}_Vout_fund"] # V

        vout_fund_data= pd.DataFrame({
            "freq": freq_vout_fund,
            f"{selected_case}_Vout_fund": vout_fund
        })


        try:
            print(f"{selected_case}----", file=sys.stderr)

            voltage_data = pd.DataFrame({
                "freq": vin_fund_data["freq"],
                "Vout_fund": vout_fund_data[f"{selected_case}_Vout_fund"],
                "Vin_fund": vin_fund_data[f"{selected_case}_Vin_fund"]
            })
            ...
            if task == "dualBW" or task == "dualGain": # dualGain FULLFILLED
                output_dict[f"{selected_case}_cg_fc1"] = cg_fc1
                output_dict[f"{selected_case}_cg_fc2"] = cg_fc2
            elif task == "singleBW" or task == "singleGain": # singleGain FULLFILLED
                output_dict[f"{selected_case}_cg_fc1"] = cg_fc1

            # NEED FREQ MASK
            if task in ["singleBW", "dualBW", "singleGains", "dualGains", "dualGainsBW"]: 
                epsilon = 1e-6
                cg_bw_fc1_lower_thres = cg_fc1_db - 3 # dB
                cg_bw_fc1_upper_thres = cg_fc1_db + epsilon # 0703 modified
                ...
                if task in ["dualBW", "dualGains", "dualGainsBW"]:
                    ...
                    cg_bw_fc2_upper_thres = cg_fc2_db + epsilon # 0703 modified
            ...
            if task in ["dualBW", "dualGains", "dualGainsBW"]:
                freq_filtered_cg_fc1 = voltage_data[cg_mask_fc1_freq_lower & cg_mask_fc1_freq_upper].copy() #dualGains_fc1
                freq_filtered_cg_fc2 = voltage_data[cg_mask_fc2_freq_lower & cg_mask_fc2_freq_upper].copy() #dualGains_fc2
                ...
                if task in ["dualBW", "dualGainsBW"]: # CG_dB within -3dB mask
            ...
            elif task in ["singleBW", "singleGains"]: # CG_dB within -3dB mask
                    ...
                    if not filtered_cg_fc.empty:
                        ...
                        if not target_freq_row.empty:
                            ...
                            if target_freq == fc1_target:
                                output_dict[f"{selected_case}_cg_bw_fc1"] = cg_bw
                            elif target_freq == fc2_target:
                                output_dict[f"{selected_case}_cg_bw_fc2"] = cg_bw
                            if bandwidth > 0:
            ...
            if task in ["dualGains", "dualGainsBW"]:
                output_dict[f"{selected_case}_cg_fc1"] = freq_filtered_cg_fc1["cg_linear"].to_list()
                output_dict[f"{selected_case}_cg_fc2"] = freq_filtered_cg_fc2["cg_linear"].to_list()
                print(f"in output_dict:\n", file=sys.stderr)
                print(f"cg_fc1: {output_dict[f'{selected_case}_cg_fc1']}", file=sys.stderr)
                print(f"cg_fc2: {output_dict[f'{selected_case}_cg_fc2']}", file=sys.stderr)
            ...
            if task == "singleGains":
                output_dict[f"{selected_case}_cg_fc1"] = freq_filtered_cg["cg_linear"].to_list()
    ...
    df = voltage_data.copy()
    df.to_csv(f"{selected_case}_voltage_data_{iteration}_{tb_date}.csv", index=False)
...
if __name__ == "__main__":
    ...
    dsfile = sys.argv[1]
    selected_case = sys.argv[2]
    iteration = sys.argv[3]
    task = sys.argv[4]
    tb_date = sys.argv[5]
    output = read_dsfile_manual(final_stage_dsfile=dsfile, selected_case=selected_case, iteration=iteration, task=task, tb_date=tb_date)
